...
summary: 本节提供将数据从以 7 模式运行的NetApp Data ONTAP迁移到ONTAP的详细步骤。 
---
= 从 7-模式到ONTAP 的数据迁移
:hardbreaks:
:allow-uri-read: 
...
2 entries were displayed.
....
. 连接到 Linux NFS 服务器。为 NFS 导出卷创建挂载点。
+
....
...
NOTE: NFSv3 卷应该被导出但不一定由 NFS 服务器挂载。如果可以安装，XCP Linux 主机客户端就会安装这些卷。
...
....
. 通过运行以下命令查询源 7-模式 NFSv3 导出 `xcp show`XCP Linux 客户端主机系统上的命令。
+
....
...
....
. 扫描源 NFSv3 导出路径并打印其文件结构的统计信息。
+
NetApp建议在 xcp 期间将源 NFSv3 导出置于只读模式 `scan`， `copy` ， 和 `sync`运营。
...
....
. 将源 7-模式 NFSv3 导出复制到目标ONTAP系统上的 NFSv3 导出。
+
....
...
. 在切换之前和切换期间，运行 `verify`再次。如果源有新的或更新的数据，则执行增量更新。运行 `xcp sync`命令。
+
....
...
. NFSv3 客户端主机需要卸载从 7-模式存储配置的源 NFSv3 导出，并从ONTAP挂载目标 NFSv3 导出。切换需要中断。
...
NOTE: NetApp假定源 7-模式卷已导出并安装在客户端系统上，并且 XCP 已安装在 Linux 系统上。 Snapshot 副本是卷的某个时间点映像，用于记录自上次 Snapshot 副本以来的增量变化。使用 `-snap`以 7 模式系统为源的选项。
...
. 扫描源 NFSv3 导出路径并打印其文件结构的统计信息。
+
NetApp建议在以下情况下将源 NFSv3 导出置于只读模式： `xcp scan` ， `copy` ， 和 `sync`运营。在 `sync`操作时，您必须通过 `-snap`具有相应值的选项。
...
(show scan and sync)
....
. 将源 7-模式 NFSv3 快照（基础）复制到目标ONTAP系统上的 NFSv3 导出。
+
....
...
. 在切换之前和切换期间，运行 `verify`再次。如果源有新的或更新的数据，则执行增量更新。如果有增量更改，请为这些更改创建一个新的 Snapshot 副本，并将该快照路径与 `-snap`同步操作的选项。
+
...
....
. NFSv3 客户端主机必须卸载从 7-模式存储配置的源 NFSv3 导出，并从ONTAP挂载目标 NFSv3 导出。此切换需要停机。
...
NOTE: NetApp假定源 NFSv4 卷已导出并安装在客户端系统上，并且 XCP 已安装在 Linux 系统上。源应该是支持 ACL 的NetApp 7-模式系统。仅支持从NetApp到NetApp 的ACL 迁移。要复制名称中带有特殊字符的文件，请确保源文件和目标文件支持 UTF-8 编码语言。
...
NOTE: NFSv4 卷应该被导出但不一定由 NFS 服务器挂载。如果可以安装，XCP Linux 主机客户端就会安装这些卷。
...
[root@localhost linux]#
....
. 通过运行以下命令查询源 NFSv4 导出 `xcp show`XCP Linux 客户端主机系统上的命令。
+
....
...
....
. 扫描源 NFSv4 导出路径并打印其文件结构的统计信息。
+
NetApp建议在以下情况下将源 NFSv4 导出置于只读模式： `xcp scan` ， `copy` ， 和 `sync`运营。
...
....
. 将源 NFSv4 导出复制到目标ONTAP系统上的 NFSv4 导出。
+
....
...
....
. 后 `copy`完成后，验证源和目标 NFSv4 导出是否具有相同的数据。运行 `xcp verify`命令。
+
....
...
NOTE: 对于此操作，需要先前的复制索引名称或编号。

. 恢复先前中断的 `copy`操作，运行 `xcp resume`命令。
+
....
...
NOTE: NetApp假定 7-模式和ONTAP系统是 SMB 许可的。已创建目标 SVM、导出源和目标 SMB 共享，并且已安装和许可 XCP。
...
....
. 将文件（带或不带 ACL）从源复制到目标 SMB 共享。以下示例显示了具有 ACL 的副本。
+
....
...
NOTE: 如果没有数据聚合，则使用存储创建一个新的数据聚合 `aggr create`命令。
